...
const City = () => {
    ...
    const handleUpdateCity = () => {
        if (window.confirm(`Are you sure you want to update this city?`)) {
            ...
            axios.put(`http://localhost:1010/U1/Cityupdate/${City_Name}`, updatedCity)
                .then(response => {
                    setCitys(citys.map(city => city.City_Name === City_Name ? response.data : city));
                                ...
                                <tbody>
                                    {citys.map((city, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="text-center align-middle" style={{ color: "#909090" }}>{city.StateId ? city.StateId.State_Name : 'N/A'}</td>
                                            <td className="text-center align-middle" style={{ color: "#909090" }}>{city.DistrictId ? city.DistrictId.District_Name : 'N/A'}</td>
